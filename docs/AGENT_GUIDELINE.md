...
- Upload DICOM mới nhận 2 chế độ tách biệt: nhiều file `.dcm` gọi `POST /dicom/upload/batch` với multipart field `files`; nhiều file `.zip` gọi `POST /dicom/upload/zip-batch` trong một request duy nhất, append từng file với cùng multipart field `file`; không upload lẫn `.zip` và `.dcm` trong cùng một lượt.
