...
- Upload DICOM mới nhận 2 chế độ tách biệt: nhiều file `.dcm` gọi `POST /dicom/upload/batch` với multipart field `files`; một file `.zip` gọi `POST /dicom/upload/zip-batch` với multipart field `file`. Không upload lẫn `.zip` và `.dcm` trong cùng một lượt.
...
- Sau upload, backend có thể trả `successfulPatients[].recentExaminations[]` với `status: NEED_REVERIFY`; đây là danh sách bác sĩ cần xác nhận trước khi chạy AI. Khi bác sĩ xác nhận, FE gom toàn bộ `successfulPatients[].recentExaminations[].images[].dicomInstanceId` thành list int và gọi `POST /ai/predict-batch` với payload `{dicomInstanceIds: [...]}`. Response là `ExaminationDto[]` có `status: AI_COMPLETED` và `images[].aiResult`.
...
- Permission doctor `UPLOAD_DICOM_IMAGE` là màn upload DICOM. Màn này cho chọn/kéo-thả nhiều file `.DCM/.dcm` hoặc một file `.zip`; nhiều file DICOM upload lên `/dicom/upload/batch`, còn ZIP upload lên `/dicom/upload/zip-batch`.
